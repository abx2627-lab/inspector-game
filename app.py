...
# MODES INTERACTIFS (1, 2, 3)
    ...
    # --------------------------------------------------
    # ÉCRAN DE FIN DE MATCH ANIMÉ (NOUVELLE CÉLÉBRATION)
    # --------------------------------------------------
        ...
        with col_panel:
            ...
            # --------------------------------------------------
                ...
                if st.button(
                    ...
                    # Calcul du score du round
                    if gap <= 1.5:
                        gain = (len(st.session_state.edges_data) - nb_ins) * 10
                        st.session_state.score_inspector += gain
                        st.session_state.last_result_msg = {
                            "type": "win",
                            "text": f"🎉 Round {st.session_state.current_round} : Trajet optimal ! (+{gain} pts pour l'Inspecteur 🔵)",
                        }
                    else:
                        st.session_state.score_adversary += 20
                        st.session_state.last_result_msg = {
                            "type": "loss",
                            "text": f"🦹 Round {st.session_state.current_round} : Trajet sous-optimal (+20 pts pour l'Adversaire 🔴)",
                        }
